...
class QuizViewModel
    ...
    constructor(
        ...
        // 현재 문제 State (읽기 전용)
        val currentQuestion: State<QuizItem?> =
            derivedStateOf {
        ...
        private val _finalLevel = mutableIntStateOf(0)
        ...
        fun submitAndGoToNext() {
            ...
            if (quiz.type == QuizType.SENTENCE_ORDER.name) {
                if (remainingWordChips.value.isNotEmpty()) {
                    val message =
                        if (_currentLanguage.value == "KOREAN") {
        ...
        fun saveTestResult() {
            val languagePref =
                com.example.chaining.domain.model.LanguagePref(
